...
module.exports = {
    ...
    async execute(interaction) {
        ...
        for (const [memberId, member] of members) {
            const hasTier1 = member.roles.cache.has(interaction.client.config.TIER_1_ROLE_ID);
            const hasTier2 = member.roles.cache.has(interaction.client.config.TIER_2_ROLE_ID);
        ...
        for (const [userId, userData] of Object.entries(usersData)) {
            ...
            const isTier2 = member.roles.cache.has(interaction.client.config.TIER_2_ROLE_ID);
            const isTier1 = member.roles.cache.has(interaction.client.config.TIER_1_ROLE_ID);
        ...
        const announcementEmbed = new EmbedBuilder()
            ...
            .setColor('#4c00b0')
            .setDescription(`<@&${interaction.client.config.TIER_1_ROLE_ID}> 
...
You can now send your new requirements in <#${interaction.client.config.TRANSACTION_CHANNEL_ID}> according to your level!!`);

        // Top donor embed
        const topDonorEmbed = topDonor ? new EmbedBuilder()
            .setTitle('<:winners:1000018706874781806>  Pro Money Maker of the Week')
            .setColor('#4c00b0')
            .setDescription(`> Congratulations to <@${topDonor}> for being the top donor this week with ⏣ ${formatNumber(topDonation)}! They will keep the <@&${interaction.client.config.PRO_MAKER_ROLE_ID}> role for the next week.`)
            .setTimestamp() : null;
...
// This function should match what's used in your real reset function
async function getWeeklyStats(client) {
    ...
    for (const [memberId, member] of members) {
        ...
        const isTier1 = member.roles.cache.has(client.config.TIER_1_ROLE_ID);
        const isTier2 = member.roles.cache.has(client.config.TIER_2_ROLE_ID);
