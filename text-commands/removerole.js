...
module.exports = {
    ...
    async execute(message, args, entities = null) {
        ...
        if ((!targetUser || !targetRole) && args.length > 0) {
            const skipWords = ['from', 'the', 'to', 'remove', 'take', 'role', 'strip', 'revoke'];
        ...
        if (!targetRole) {
            return message.reply('❌ Please specify a valid role to remove.');
        }
        if (!targetUser) {
            return message.reply('❌ Please specify a valid user to remove the role from.');
        }
        ...
        if (!targetMember.roles.cache.has(targetRole.id)) {
            return message.reply(`⚠️ ${targetUser.username} doesn't have the ${targetRole.name} role.`);
        }
        ...
        if (targetRole.position >= message.member.roles.highest.position) {
            return message.reply('❌ You cannot remove this role as it is higher than or equal to your highest role.');
        }
        ...
        try {
            ...
            return message.reply(`✅ Successfully removed **${targetRole.name}** from **${targetUser.username}**.`);
        } catch (error) {
            console.error('Error removing role:', error);
            return message.reply('❌ There was an error trying to remove the role. Please try again.');
        }
    },
};