...
if [ $PULL_EXIT_CODE -ne 0 ]; then
    ...
    send_discord_message "<a:tickloop:926319357288648784> Successfully pulled changes from repository"
    
    # Deploy slash commands first
    cd /home/ubuntu/spectre && node deploy.js > /dev/null 2>&1
    DEPLOY_EXIT_CODE=$?
    
    if [ $DEPLOY_EXIT_CODE -eq 0 ]; then
        send_discord_message "<a:tickloop:926319357288648784> Slash commands deployed successfully"
    else
        send_discord_message "❌ Error: Failed to deploy slash commands"
    fi
    
    # Use npx to run PM2 - restart after deployment
    cd /home/ubuntu/spectre && npx pm2 restart spectre --update-env > /dev/null 2>&1
    PM2_EXIT_CODE=$?
    
    if [ $PM2_EXIT_CODE -eq 0 ]; then
        send_discord_message "<a:tickloop:926319357288648784> Bot restarted successfully with new changes"
    else
